...
predsw = dcast(
  preds,
  task + cv + fold + week + date + symbol + cv + truth  ~ learner,
  value.var = "response"
)
```
...
setorder(portfolio_returns, cv, variable, week)
```

```{r}
calculatePortfolioStats <- function(portfolioReturns) {
  # portfolioReturns = portfolios[, portfolio_returns][[1]] # OLD WAY
  portfolioReturns = copy(portfolio_equal)
  portfolioReturns = as.xts.data.table(portfolioReturns[, .(week, returns)])
...
calculatePortfolioStats <- function(portfolio_ret) {
  # portfolio_ret = portfolio_returns[cv == 1057 & variable == "median_resp", .(week, returns_equal)]
...
portfolio_returns[1:5000, calculatePortfolioStats(.SD[, .(week, returns_equal)]), 
                  by = .(cv, variable)]

---
title: "Primjena modela strojnog učenja za predviđanje očekivanih prinosa dionica u RH"
subtitle: "Application of Machine Learning Models for Predicting Expected Stock Returns in Croatia"
author:
  - name: "Mislav Šagovac"
  - name: "Luka Šikić"
  - name: "Petra Palić"
...
lang: hr
abstract: |
  Ovaj rad istražuje primjenu modela strojnog učenja za predviđanje očekivanih
  prinosa dionica na Zagrebačkoj burzi (ZSE) u razdoblju od 2000. do 2024. godine.
  Koristi se velik skup prediktora izvedenih iz dnevnih podataka o trgovanju te
  ugniježđena metoda križne validacije s pomičnim prozorom (NRWCV). Uspoređuju se
  četiri modela — Random Forest, XGBoost, GLMNET i neuronska mreža (NNET) — kao i
  njihovi jednostavni ansambli. Rezultati pokazuju da modeli stabala odlučivanja
  (Random Forest) i neuronske mreže ostvaruju najbolji odnos rizika i prinosa, a
  ansambli predikcija dodatno poboljšavaju uspješnost na relativno nelikvidnom i
  plitkom hrvatskom tržištu kapitala.
keywords:
  - strojno učenje
  - predviđanje prinosa
  - Zagrebačka burza
  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \fancyhead[L]{\small\color{slate}\itshape Strojno učenje za predviđanje prinosa dionica na ZSE}
...
#| eval: true
#| echo: false
#| label: setup-display-values

# NOTE ON REPRODUCIBILITY
# -----------------------
# The analysis chunks below depend on proprietary trading data and a batchtools
# results registry that live outside this repository (originally under F:/zse/).
# To keep the document fully renderable, code chunks are shown but NOT executed
# (`eval: false` is set globally in the YAML header). The few inline values that
# appear in the running text are assigned literal, paper-reported figures here so
# the prose reads correctly. Re-run with the original data and `eval: true` to
# reproduce all tables and figures.

first_date    <- "03.01.2000."   # početak uzorka
last_date     <- "29.03.2024."   # kraj uzorka
n             <- "~1.300.000"    # broj OHLCV opservacija (dnevna frekvencija)
n_after       <- "~150.000"      # broj opservacija nakon agregacije na tjednu frekvenciju
n_predictors  <- "preko 500"     # broj konstruiranih prediktora
```
...
::: {#fig-ncompanies}
> *Slika se generira iz izvornih podataka (`F:/zse/prices.csv`) pokretanjem gornjeg
> koda s `eval: true`. U ovoj inačici dokumenta podaci nisu uključeni pa je prikazan
> samo kôd.*
...
::: {#fig-portfolio-returns}
> *Krivulja kapitala generira se iz rezultata modela (registry pod `F:/zse/`)
> pokretanjem gornjeg koda s `eval: true`. U ovoj inačici dokumenta rezultati nisu
> uključeni pa je prikazan samo kôd.*

Krivulja kapitala portfolia
:::
